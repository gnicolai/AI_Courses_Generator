...
async def espandi_contenuti_corso(corso_id: str, parametri_espansione: Dict[str, Any]) -> Dict[str, Any]:
    ...
    pausa_tra_capitoli = parametri_espansione.get("pausa_tra_capitoli", 0)  # Secondi di pausa tra capitoli
    pausa_tra_sezioni = parametri_espansione.get("pausa_tra_sezioni", 3)    # Secondi di pausa tra sezioni (default: 3s)
    ...
    for i, capitolo in enumerate(corso["scaletta"]["capitoli"]):
        ...
        try:
            ...
            for j, sezione in enumerate(sezioni):
                ...
                # Se non è la prima sezione, facciamo una pausa per evitare rate limiting
                if j > 0 and pausa_tra_sezioni > 0:
                    logger.info(f"Pausa di {pausa_tra_sezioni} secondi tra le sezioni...")
